...
class SearchController {
  ...
  searchByTitle(formData, page = 1) {
    ...
    const query = formData.get('movie-title')
    ...
    this.apiService.searchByTitle(query, page)
      .then(json => {
        this.paginationService.update(json)
        const filteredResults = this.filterService.applyFilters(json.results, filters)
  ...
  searchByFilters(formData = null, page = 1) {
    ...
    const filters = formData ? this.filterService.extractFiltersFromFormData(formData) : {}

    this.lastSearchType = 'filters'
    this.lastSearchParams = formData

    this.apiService.searchByFilters(filters, page)
      .then(json => {
        this.paginationService.update(json)
  ...
  searchWithAi(formData) {
    ...
    this.apiService.searchWithAi(userDescription)
      .then(movieInfo => {
  ...
  searchMovieByAiResult(movieInfo, page = 1) {
    ...
    this.apiService.searchByAiResult(movieInfo, page)
      .then(json => {
        this.paginationService.update(json)
        this.updateMoviesData(json.results || [], page)
        this.renderResults()
  ...
  bindEvents() {
    ...
    this.loaderService.loadMoreButtonElement.addEventListener('click', () => this.loadMoreResults())
  }
}
